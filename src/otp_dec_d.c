...
//inspired from handler in The Linux Programming Interface p. 1244
	...
	for (;;)
	{
		...
		switch(spawnpid)
		{
			...
			case 0:
			{
				...
				int textFlag = 0;
				int keyFlag = 0; 
				//get the indices of each char corresponding with usableChars
				for(i = 0; i < strlen(textContents); i++)
				{
					...
					//calculate for decryption
					//then use the value to get the index and store the char
					totalAscii = (textAscii - keyAscii);
					//check for negative value
					if (totalAscii < 0)
					{
						totalAscii += 27;
					}
					totalAscii = totalAscii % 27;
