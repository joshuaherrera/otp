...
int main(int argc, char *argv[])
{
	...
	//to ensure connection with encryption send identifying chars at start.
	//remember when checking validity, receive whole file first, then verify
	// ^ == encrypty * == decrypt
	strcpy(buffer, "*");
	...
	//this is where we pass the file to get decrypted.
	...
	// get the decrypted message and spew to screen
	...
	buffer[endLoc] = '\0';
	//checks for rejected connection
	if (strcmp(buffer, "~~") == 0)
	{
